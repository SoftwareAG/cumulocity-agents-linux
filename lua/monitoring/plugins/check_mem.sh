...
MEMFREE_PERCENT=$(($FMEM * 100 / $ALLMEM))


# output with or without performance data
if [ "$perform" = "yes" ]; then
  OUTPUTP="free system memory: $FMEM MB ($MEMFREE_PERCENT %) | free memory="$FMEM"MB;$int_warn;$int_crit;0"
else
  OUTPUT="free system memory: $FMEM MB ($MEMFREE_PERCENT %)"
fi
...
if [ -n "$int_warn" -a -n "$int_crit" ]; then
  ...
  if (( $MEMFREE_PERCENT <= $int_crit )); then
    err=2
  elif (( $MEMFREE_PERCENT <= $int_warn )); then
    err=1
  fi
