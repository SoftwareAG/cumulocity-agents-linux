...
public:
        ...
        long timeout() const {return tv.tv_sec * 1000000 + tv.tv_usec;}
        std::string errMsg() {return errmsg + modbus_strerror(errno);}
        int data(int type, int index) const {
                switch (type) {
                case 0: return coData[index];
                case 1: return diData[index];
                case 2: return hrData[index];
                case 3: return irData[index];
                default: return 0;
                }
        }
        int size(int type) const {
                switch (type) {
                case 0: return coData.size();
                case 1: return diData.size();
                case 2: return hrData.size();
                case 3: return irData.size();
                default: return 0;
                }
        }
        int setSlave(int slave);
        int connect();
        void close() {modbus_close(ctx);}
...
{
public:
        ModbusRTU(const char *device, int baud, char par, int data,
                  int stop): ModbusBase() {
                srInfo(std::string("Modbus: new RTU ") + device + " " +
                       std::to_string(baud) + " " + par + std::to_string(data)
                       + ":" + std::to_string(stop));
        ...
        void setConf(const char *dev, int baud, char par, int data, int stop) {
                modbus_free(ctx);
                srInfo(std::string("Modbus: RTU setConf: ") + dev + " " +
                       std::to_string(baud) + " " + par + std::to_string(data)
                       + ":" + std::to_string(stop));
...
public:
        ModbusTCP(const char *ip, int port): ModbusBase() {
                srInfo(std::string("Modbus: new TCP ") + ip + ":" +
                       std::to_string(port));
