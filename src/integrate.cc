...
#define HTTP_INTEGRATE_TIMEOUT_IN_SECONDS 60
...
static void getHardware(string &model, string &revision)
{
    ...
    if (model.empty())
    {
        ...
        for (string line; getline(in, line);)
        {
            ...
            {
                copy_if(line.begin() + 8, line.end(), back_inserter(model), foo);
            } else if (line.compare(0, 8, "Revision") == 0)
            {
                copy_if(line.begin() + 8, line.end(), back_inserter(revision), foo);
            }
        }
    }
}
...
int Integrate::integrate(const SrAgent &agent, const string &srv, const string &srt)
{
    ...
    http.setTimeout(HTTP_INTEGRATE_TIMEOUT_IN_SECONDS);
    int c = 0;

    while ((c = registerSrTemplate(http, xid, srt) == -1) && NOCONNECT(http.errNo))
    {
        sleep(2);
        wdt.kick();
    }
    ...
    if (r.size() && (r[0].second == "50"))
    {
        ...
        if ((r.size() == 3) && (r[0].second == "801"))
        {
            ...
            if (!model.empty())
            {
                s += "\n310," + id + "," + model + "," + agent.deviceID() + "," + rev;
            }
